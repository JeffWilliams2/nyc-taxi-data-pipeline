...
class AdhocRequestConfig(dg.Config):
    filename: str = "default-request.json"
...
def adhoc_request(config: AdhocRequestConfig, database: DuckDBResource) -> None:
    ...
    # Validate that start_date and end_date are not empty
    if not config.start_date or not config.end_date:
        raise ValueError("start_date and end_date must be provided and cannot be empty")
